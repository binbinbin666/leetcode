...
public class 第五题走迷宫bfs {
	...
	public static String BFS(int[][] labyrinth, int row, int column) {
		...
		queue.offer(node);//队列添加元素
		while (!queue.isEmpty()) {
			Node head = queue.poll();//返回第一个元素并在队列中删除
			stack.push(head); // 用于回溯路径
			visit[head.x][head.y] = 1;//标记为已经访问
			for (int i = 0; i < 4; i++) {
...
class Node {
	int x, y;//当前坐标
	int step;//步数
	int preX, preY;//上一步的坐标
	String direction;//上一点到达这个点要走的方向
